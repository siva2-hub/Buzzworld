...
public class RepairPages extends App {
	...
	public void createRMA() throws Exception {
		...
		Thread.sleep(500);
		wait.until(
				ExpectedConditions.visibilityOfElementLocated(By.xpath("//*[contains(text(),'Zummo Meat Co Inc')]")));
		quotes.selectDropDown("Zummo Meat Co Inc");
		wait.until(
				ExpectedConditions.invisibilityOfElementLocated(By.xpath("//*[contains(@class, 'opacity-on-load')]")));
		Thread.sleep(1500);
		...
		// Thread.sleep(1500);
		// wait.until(ExpectedConditions.invisibilityOfElementLocated(By.xpath("//*[@viewBox='0
		// 0 16 16']")));
		// act.sendKeys(Keys.TAB).build().perform();
	...
	public boolean verifyCreateRMA() throws Exception {
		...
		if (actText.toLowerCase().contains(expText.toLowerCase())) {
			res = true;
			Object status[] = { "REPAIRS_001_VerifyCreateRMA", actText, expText, "RepairsPage", "Passed",
					java.time.LocalDate.now().toString() };
			quotes.values(status);
		} else {
			res = false;
			Object status[] = { "REPAIRS_001_VerifyCreateRMA", actText, expText, "RepairsPage", "Failed",
					java.time.LocalDate.now().toString() };
			quotes.values(status);
		}
		return res;
	...
	public boolean verifySelectItemsToRepair() throws Exception {
		...
		if (!actText.equals(expText)) {
			res = true;
			Object status[] = { "REPAIRS_002_VerifySelectItemToRepair", actText, expText, "RepairsPage", "Passed",
					java.time.LocalDate.now().toString() };
			quotes.values(status);
		} else {
			res = false;
			Object status[] = { "REPAIRS_002_VerifySelectItemToRepair", actText, expText, "RepairsPage", "Failed",
					java.time.LocalDate.now().toString() };
			quotes.values(status);
		}
		return res;
	...
	public boolean verifyAddNewItem(String env) throws Exception {
		...
		if (!actText.equals(expText)) {
			res = true;
			Object status[] = { "REPAIRS_020_VerifyAddNewItem", actText, expText, "RepairsPage", "Passed",
					java.time.LocalDate.now().toString(), env };
			App.values1(status);
		} else {
			res = false;
			Object status[] = { "REPAIRS_020_VerifyAddNewItem", actText, expText, "RepairsPage", "Failed",
					java.time.LocalDate.now().toString(), env };
			App.values1(status);
		}
		return res;
	...
	public void assignLocation() throws Exception {
		...
		Thread.sleep(1200);
		Actions act = new Actions(driver);
		WebElement editIcon = driver.findElement(By.xpath("//*[@class='quantity-parent']"))
		...
		if (driver.findElement(By.name("serial_no")).getAttribute("value").length() == 0) {
			...
			driver.findElement(By.name("serial_no"))
		...
		driver.findElement(By.xpath("//*[@placeholder= 'Type here']"))
				.sendKeys("Internal Item Notes While Assign Location");
		Thread.sleep(1200);
		driver.findElement(By.xpath("//*[@class='side-drawer open']")).findElement(By.tagName("button")).click();
		App.spinner();
		Thread.sleep(1300);
	...
	public void assignTechnician() throws Exception {
		...
		List<WebElement> btns = driver.findElement(By.xpath("//*[@class='side-drawer open']"))
	...
	public void createQuoteFromRepair() throws Exception {
		...
		wait.until(
				ExpectedConditions.visibilityOfElementLocated(By.xpath("//*[text()='Submit for internal approval']")));
		Thread.sleep(2000);
	}
	...
	public void assignToQC() throws Exception {
		...
		wait.until(ExpectedConditions.invisibilityOfElementWithText(
				By.xpath("//*[text()='Submit for internal approval']"), "Submit for internal approval"));
		Thread.sleep(1700);
		...
		Thread.sleep(1400);
		driver.findElements(By.xpath("//*[contains(@class,'border-bottom')]")).get(1).findElement(By.tagName("h4"))
				.click();
	...
	public boolean verifyAssignToQC() throws Exception {
		...
		if (actText.toLowerCase().contains(expText.toLowerCase())) {
			res = true;
			Object status[] = { "REPAIRS_009_VerifyAssignToQC", actText, expText, "RepairsPage", "Passed",
					java.time.LocalDate.now().toString() };
			quotes.values(status);
		} else {
			res = false;
			Object status[] = { "REPAIRS_009_VerifyAssignToQC", actText, expText, "RepairsPage", "Failed",
					java.time.LocalDate.now().toString() };
			quotes.values(status);
		}
		return res;
	...
	public boolean verifyQCCheckList(String data, String env) throws Exception {
		...
		if (actText.toLowerCase().contains(expText.toLowerCase())) {
			res = true;
			Object status[] = { "REPAIRS_010_VerifyQCCheckList_as_" + data, actText, expText, "RepairsPage", "Passed",
					java.time.LocalDate.now().toString(), env };
			App.values1(status);
		} else {
			res = false;
			Object status[] = { "REPAIRS_010_VerifyQCCheckList_" + data, actText, expText, "RepairsPage", "Failed",
					java.time.LocalDate.now().toString(), env };
			App.values1(status);
		}
		return res;
	...
	public boolean verifyQCCheckListStatusAsFail() throws Exception {
		...
		if (actText.toLowerCase().equals(expText.toLowerCase())) {
			res = true;
			Object status[] = { "REPAIRS_017_VerifyQCCheckListStatusAsFail", actText, expText, "RepairsPage", "Passed",
					java.time.LocalDate.now().toString() };
			quotes.values(status);
		} else {
			res = false;
			Object status[] = { "REPAIRS_017_VerifyQCCheckListStatusAsFail", actText, expText, "RepairsPage", "Failed",
					java.time.LocalDate.now().toString() };
			quotes.values(status);
		}
		return res;
	...
	public boolean fileUpload(String env) throws Exception {
		...
		driver.findElement(By.xpath("//*[@type='file']"))
				.sendKeys("/home/enterpi/Pictures/Screenshot from 2023-04-18 10-35-23.png");
		Thread.sleep(2000);
		...
		if (sta) {
			res = true;
			Object status[] = { "REPAIRS_017_VerifyFileUpload_Repairs", "Document Uploaded successfully", "",
					"RepairsPage", "Passed", java.time.LocalDate.now().toString(), env };
			App.values1(status);
		} else {
			res = false;
			Object status[] = { "REPAIRS_017_VerifyFileUpload_Repairs", "Document Uploading Failed.!", "",
	...
	public void filters(String data1, String data2, String data3) throws Exception {
		...
		App.spinner();
		Thread.sleep(1300);
		List<WebElement> inputs = driver.findElement(By.xpath("//*[@class='side-drawer open']"))
				.findElements(By.xpath("//*[@type='text'][@autocapitalize='none']"));
		...
		for (int i = 0; i < inputs.size(); i++) {
			...
			Actions act = new Actions(driver);
			act.sendKeys(Keys.ENTER).build().perform();
			inputs = driver.findElement(By.xpath("//*[@class='side-drawer open']"))
					.findElements(By.xpath("//*[@type='text'][@autocapitalize='none']"));
		}
		List<WebElement> btns = driver.findElement(By.xpath("//*[@class='side-drawer open']"))
				.findElements(By.tagName("button"));
	...
	public boolean verifyFilters(String data, String data1, String data2, String env) throws Exception {
		String compName = "123 E Doty Corporation";
		String salesP = "Dallas House";
		String status1 = "Receiving";
		boolean res = false;
		this.filters(compName, salesP, status1);
		System.out.println("grid text after apply the filter "
				+ driver.findElement(By.xpath("//*[@class='ag-center-cols-viewport']")).getText());
		if (driver.findElement(By.xpath("//*[@class='ag-center-cols-viewport']")).getText().equalsIgnoreCase("")) {
			res = false;
			Object status[] = { "REPAIRS_019_VerifyFilters", "Repair Request Not Found for Applied filters", "",
					"RepairsPage", "Failed", java.time.LocalDate.now().toString(), env };
			App.values1(status);
		} else {

			List<WebElement> txts = driver.findElement(By.xpath("//*[@class='ag-center-cols-container']"))
					.findElements(By.xpath("//*[@row-index='0']"));
			...
			if (compName.equalsIgnoreCase(actComp) && salesP.equalsIgnoreCase(actSp)
					&& status1.equalsIgnoreCase(actStatus)) {
				res = true;
				Object status[] = { "REPAIRS_019_VerifyFilters", actComp + " " + actSp + " " + actStatus,
						compName + " " + salesP + " " + status1, "RepairsPage", "Passed",
						java.time.LocalDate.now().toString(), env };
				App.values1(status);
			} else {
				res = false;
				Object status[] = { "REPAIRS_019_VerifyFilters", actComp + " " + actSp + " " + actStatus,
						compName + " " + salesP + " " + status1, "RepairsPage", "Failed",
						java.time.LocalDate.now().toString(), env };
				App.values1(status);
			}
		}
	...
	public boolean verifyFilterStateMaintanance(String env) throws Exception {
		...
		if (ele.isDisplayed()) {
			res = true;
			Object status[] = { "REPAIRS_028_VerifyFilterStateMaintanance",
					"filters  are stabled after refresh the page",
					"", "RepairsPage", "Passed", java.time.LocalDate.now().toString(), env };
			App.values1(status);
		} else {
			res = false;
			Object status[] = { "REPAIRS_028_VerifyFilterStateMaintanance",
					"filters  are not stabled after refresh the page",
					"", "RepairsPage", "Failed", java.time.LocalDate.now().toString(), env };
			App.values1(status);
		}
		return res;
	...
	public void assignToQC_SO_Created() throws Exception {
		...
		editIcon.click();
		driver.findElement(By.name("serial_no"))
				.sendKeys(java.time.LocalTime.now().toString().replace("-", "").replace(":", "").replace(".", ""));
		driver.findElement(By.xpath("//*[@title='Save Changes']")).click();
		driver.findElement(By.xpath("//*[@class='side-drawer open']")).findElement(By.tagName("button")).click();
		Thread.sleep(2000);
		...
		wait.until(ExpectedConditions.visibilityOfElementLocated(
				By.xpath("//*[contains(text(), '" + java.time.LocalDate.now().toString().replace("-", "") + "')]")));
		// Go to Quotes Module
		Thread.sleep(1400);
		driver.findElement(
				By.xpath("//*[contains(text(), '" + java.time.LocalDate.now().toString().replace("-", "") + "')]"))
	...
	public boolean verifyAssignToQCIsDisplayedOrNotWhenQuoteStatusHasWon() throws Exception {
		...
		String expRMAStatus = "IN PROGRESS";
		String expItemStatus = "Assign to QC";
		boolean res = false;
		...
		if (rMAStatus.contains(expRMAStatus) && itemStatus.contains(expItemStatus)) {
			res = true;
			Object status[] = { "REPAIRS_021_VerifyAssignToQCIsDisplayedOrNotWhenQuoteStatusHasWon",
					"act item status is " + itemStatus + "exp item status is " + expItemStatus,
					"act RMA status is " + rMAStatus + "exp RMA status is " + expRMAStatus, "RepairsPage", "Passed",
					java.time.LocalDate.now().toString() };
			quotes.values(status);
		} else {
			res = false;
			Object status[] = { "REPAIRS_021_VerifyAssignToQCIsDisplayedOrNotWhenQuoteStatusHasWon",
					"act item status is " + itemStatus + "exp item status is " + expItemStatus,
					"act RMA status is " + rMAStatus + "exp RMA status is " + expRMAStatus, "RepairsPage", "Failed",
					java.time.LocalDate.now().toString() };
			quotes.values(status);
		}
		return res;
	...
	public void assignToQC_WhenReviseTheQuote() throws Exception {
		...
		Thread.sleep(1400);
		driver.findElements(
				By.xpath("//*[contains(@title, '" + java.time.LocalDate.now().toString().replace("-", "") + "')]"))
				.get(1).click();
		wait.until(
				ExpectedConditions.visibilityOfElementLocated(By.xpath("//*[text()='Submit for internal approval']")));
		Thread.sleep(1400);
		...
		wait.until(ExpectedConditions.invisibilityOfElementWithText(By.xpath("//*[text()='Approve']"), "Approve"));
		Thread.sleep(2200);
		driver.findElement(By.xpath("//*[@class='quote-num-and-status']")).findElements(By.tagName("button")).get(1)
				.click();
		driver.findElement(By.xpath("//*[@class='link-icon-text']")).click();
		Thread.sleep(2000);
		// Quote Won
		...
		wait.until(ExpectedConditions
				.invisibilityOfElementWithText(By.xpath("/html/body/div[1]/div/div[3]/div[2]/button[1]"), "Won"));
		Thread.sleep(1400);
		// Back to Repair Module
		driver.findElements(By.xpath("//*[contains(@class,'border-bottom')]")).get(1).findElement(By.tagName("h4"))
				.click();
	...
	public boolean verifyAssignToQC_WhenReviseTheQuote() throws Exception {
		...
		boolean res = false;
		Thread.sleep(1500);
		String itemStatus = driver.findElements(By.xpath("//*[contains(@class,'action-item icon-bg-hover')]")).get(3)
				.getText();
		if (rMAStatus.contains(expRMAStatus) && itemStatus.contains(expItemStatus)) {
			res = true;
			Object status[] = { "REPAIRS_022_VerifyAssignToQC_WhenReviseTheQuote",
					"act item status is " + itemStatus + "exp item status is " + expItemStatus,
					"act RMA status is " + rMAStatus + "exp RMA status is " + expRMAStatus, "RepairsPage", "Passed",
					java.time.LocalDate.now().toString() };
			quotes.values(status);
	...
	public void repairsModule() throws Exception {
		...
		// Assign Location
		List<WebElement> btn = driver.findElement(By.xpath("//*[@id='repair-items']"))
				.findElements(By.xpath("//*[contains(@class,'action-item icon-bg-hover')]"));
		btn.get(0).click();
		Thread.sleep(1000);
		Thread.sleep(1500);
		Actions act = new Actions(driver);
		...
		// Assign Technician
		wait.until(
				ExpectedConditions.visibilityOfElementLocated(By.xpath("//*[contains(text(),'Assign Technician')]")));
		...
		this.toastContainer("Accept");
		// btns =
		// driver.findElement(By.className("toast-action")).findElements(By.tagName("button"));
		// for(int i=0;i<btns.size();i++)
		// {
		...
		Thread.sleep(2000);
		this.toastContainer("Proceed");
		wait.until(ExpectedConditions.invisibilityOfElementWithText(By.xpath("/html/body/div/div/div[3]/div[2]/button"),
				"Submit for internal approval"));
		Thread.sleep(1700);
		price.clickButton("Approve");
		Thread.sleep(1200);
		this.toastContainer("Approve");
		wait.until(ExpectedConditions
				.invisibilityOfElementWithText(By.xpath("/html/body/div[1]/div/div[3]/div[2]/button"), "Approve"));
		Thread.sleep(1500);
		driver.findElement(By.xpath("//*[@class='quote-num-and-status']")).findElement(By.tagName("button")).click();
		...
		if (actText.toLowerCase().contains(expText.toLowerCase())) {
			...
			Object status[] = { "REPAIRS_010_VerifyQCCheckList", actText, expText, "RepairsPage", "Passed",
					java.time.LocalDate.now().toString() };
			quotes.values(status);
		} else {
			Object status[] = { "REPAIRS_010_VerifyQCCheckList", actText, expText, "RepairsPage", "Failed",
					java.time.LocalDate.now().toString() };
			quotes.values(status);
		}
	}
