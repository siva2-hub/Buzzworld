...
public class AllModules extends App {
	...
	public void logoutCheckURLRedirectsOrNot(String env) throws Exception {
		...
		String url[] = { "quote_for_parts", "organizations", "contacts", "pricing", "discount-codes", "special-pricing",
				"repair-request", "jobs", "orders", "part-purchase", "inventory", "account-type", "branches",
				"classification", "contact_types", "industry", "po_min_qty", "product_class", "qc_control",
				"quote-approval", "quote-type", "regions", "sales_potential", "terms-conditions", "territory", "users",
				"user_roles",
		...
		for (int i = 0; i < url.length; i++) {
			...
			String openURL = driver.getCurrentUrl()
					.replace(driver.getCurrentUrl().substring(39, driver.getCurrentUrl().length()), url[i]);
			...
			if (openURL.equals(openedURL)) {
				Object status[] = {
						"0" + count + "_Check_" + url[i].toUpperCase()
								+ "_URLs_Redirects_To_Same_Or_Not_After_Logout_and_Login",
						"Actual URL is " + openedURL,
	...
	public void linksRedirectsOrNot(String env) throws Exception {
		...
		String url[] = { "organizations", "contacts", "pricing", "discount-codes", "special-pricing", "repair-request",
				"quote_for_parts", "jobs", "orders", "part-purchase", "inventory", "account-type", "branches",
				"classification", "contact_types", "industry", "po_min_qty", "product_class", "qc_control",
				"quote-approval", "quote-type", "regions", "sales_potential", "terms-conditions", "territory", "users",
				"user_roles",
				"vendors", "warehouse", "zipcodes", "past-due-invoices", "point-of-sales" };
		String data = "quote_for_parts";
		int count = 1;
		for (int i = 0; i < url.length; i++) {
			...
			if (actURL.equals(expURL)) {
				Object status[] = { "Check_URLS_0" + count, "Actual URL is " + actURL, "Expected URL is " + expURL,
						url[i].toUpperCase(), "Passed",
						java.time.LocalDateTime.now().toString(), env };
				App.values1(status);
			} else {
				Object status[] = { "Check_URLS_0" + count, "Actual URL is " + actURL, "Expected URL is " + expURL,
						url[i].toUpperCase(), "Failed",
						java.time.LocalDateTime.now().toString(), env };
				App.values1(status);
			}
			count++;
	...
	public void verify_search_with_plus_icon(String cust_name, String supplier, String stock_code, String env)
			throws Exception {
		...
		App.clearFilter();
		Thread.sleep(1300);
		driver.findElement(By.xpath("//*[@placeholder = 'RMA ID / Company Name / Sales Person / Items / Vendor Code']"))
				.sendKeys(stock_code);
	...
	public void verify_conditions(String tc_name, String exp_text, int g_count, String env) throws Exception {
		...
		try {
			...
			if (act_text.equals(exp_text)) {
				Object status[] = { tc_name, "Actual text is " + act_text, "Expected text is " + exp_text,
						"Search Stock + Code", "Passed",
						java.time.LocalDateTime.now().toString(), env };
				App.values1(status);
			} else {
				Object status[] = { tc_name, "Actual text is " + act_text, "Expected text is " + exp_text,
						"Search Stock + Code", "Failed",
						java.time.LocalDateTime.now().toString(), env };
				App.values1(status);
			}
		} catch (Exception e) {
	...
	public void repairsModule(String stCode, String env) throws Exception {
		...
		try {
			...
			if (driver.findElement(By.xpath("//*[@data-size = 'large']")).getAttribute("data-checked")
		...
		if (isSelected) {
			Object status[] = { "REPAIRS_001_Verify_Display All Items", "By default isSeleted status is " + isSelected,
					"", "RepairsPage",
		...
		try {
			...
			driver.findElement(By.xpath("//*[@placeholder='Serial No']"))
					.sendKeys(java.time.LocalTime.now().toString().substring(0, 8).replace(":", ""));
		...
		expText = "CHECKED-IN";
		actText = driver
				.findElement(By.xpath("//*[@style = 'white-space: nowrap; max-width: 100%; text-overflow: ellipsis;']"))
				.getText();
		if (actText.toLowerCase().contains(expText.toLowerCase())) {
		...
		actText = driver
				...
				.getText();
		if (actText.toLowerCase().contains(expText.toLowerCase())) {
			Object status[] = { "REPAIRS_006_VerifyEvaluateItem", actText, expText, "RepairsPage", "Passed",
					java.time.LocalDateTime.now().toString(), env };
			App.values1(status);
		} else {
			Object status[] = { "REPAIRS_006_VerifyEvaluateItem", actText, expText, "RepairsPage", "Failed",
					java.time.LocalDateTime.now().toString(), env };
			App.values1(status);
		}
		...
		act.moveToElement(driver.findElements(By.xpath("//*[contains(@src,'themecolorEdit')]")).get(1)).build()
				.perform();
		act.moveToElement(driver.findElements(By.xpath("//*[contains(@src,'themecolorEdit')]")).get(1)).build()
				.perform();
		...
		wait.until(
				ExpectedConditions.invisibilityOfElementWithText(By.xpath("//*[text()='" + btn_name + "']"), btn_name));
		Thread.sleep(1700);
		...
		wait.until(
				ExpectedConditions.visibilityOfElementLocated(By.xpath("//*[text()='Submit for Customer Approval']")));
		Thread.sleep(1500);
		...
		if (driver.getCurrentUrl().contains("staging")) {
			...
			if (test) {
				...
				wait.until(ExpectedConditions
						.visibilityOfElementLocated(By.xpath("//*[@placeholder = 'Enter Stock Description']")));
				String list_price = driver.findElement(By.xpath("//*[@placeholder = 'Enter List Price']"))
						.getAttribute("value");
				if (list_price.contains("0.")) {
					for (int i = 0; i < list_price.length(); i++) {
						driver.findElement(By.xpath("//*[@placeholder = 'Enter List Price']"))
				...
				driver.findElements(By.xpath("//*[text() = 'Create']")).get(1).click();
				wait.until(ExpectedConditions
						.invisibilityOfElementLocated(By.xpath("//*[@style = 'animation-delay: 0ms;']")));
				Thread.sleep(1600);
			}
			String serverMsg = "";
			...
			if (server) {
				Object status[] = { "REPAIRS_014_VerifyCreateSalesOrder_FromRepair", serverMsg, "", "RepairsPage",
						"Failed",
						java.time.LocalDateTime.now().toString(), env };
				App.values1(status);
				...
				wait.until(ExpectedConditions
						.visibilityOfElementLocated(By.xpath("//*[text() = 'Sales Order Information']")));
				String ord_id = driver.findElement(By.xpath("//*[@class = 'id-num']")).getText().replace("#", "");
				Object status1[] = { "REPAIRS_014_VerifyCreateSalesOrder_FromRepair",
						"Sales order created with ID " + ord_id, "", "RepairsPage", "Passed",
						java.time.LocalDateTime.now().toString(), env };
				App.values1(status1);
				App.displayPopUp("REPAIRS_016_VerifyCreateJobFromRepair");
				List<WebElement> tabIds = driver.findElement(By.xpath("//*[@style = 'margin-left: 24px;']"))
						.findElements(By.tagName("h4"));
				if (tabIds.size() == 3) {
					tabIds.get(2).click();
					App.spinner();
					Thread.sleep(1500);
					wait.until(
							ExpectedConditions.visibilityOfElementLocated(By.xpath("//*[text() = 'Job Information']")));// *[text()
																														// =
																														// '224290']
					wait.until(
							ExpectedConditions.visibilityOfElementLocated(By.xpath("//*[text() = '" + ord_id + "']")));
					if (driver.findElement(By.xpath("//*[text() = 'Job Information']")).isDisplayed()) {
						jobId = driver.findElement(By.xpath("//*[@class = 'quote-num-and-status']")).getText()
								.replace("#", "");
						Object status[] = { "REPAIRS_016_VerifyCreateJobFromRepair",
								"Job created with Job Id is " + jobId, "", "RepairsPage", "Passed",
								java.time.LocalDateTime.now().toString(), env };
						App.values1(status);
						...
						Object status[] = { "REPAIRS_016_VerifyCreateJobFromRepair", "Jon Not Created", "",
								"RepairsPage", "Failed",
								java.time.LocalDateTime.now().toString(), env };
						App.values1(status);
					}
					...
					Object status[] = { "REPAIRS_016_VerifyCreateJobFromRepair", "Jon Not Created",
							"count of ids is" + tabIds.size(), "RepairsPage", "Failed",
							java.time.LocalDateTime.now().toString(), env };
					App.values1(status);
				}
			}
			...
			if (partsPurchseIcon) {
				...
				Thread.sleep(1200);
				// price.closeIcon();
				Object status[] = { "REPAIRS_015_Verify_Parts_Purchase_Icon_isDisplayed_OrNot",
						"Parts Purchase Icon is Displayed", "",
						"RepairsPage", "Passed", java.time.LocalDateTime.now().toString(), env };
				App.values1(status);
				this.createPartsPurchase("REPAIRS_024_VerifyCreate_PartsPurchase", jobId, 1, env);
			} else {
				Object status[] = { "REPAIRS_015_Verify_Parts_Purchase_Icon_isDisplayed_OrNot",
						"Parts Purchase Icon is not Displayed ", "",
						"RepairsPage", "Failed", java.time.LocalDateTime.now().toString(), env };
				App.values1(status);
			}
			...
			js.executeScript("arguments[0].scrollIntoView(true);",
					driver.findElement(By.xpath("//*[text()='Related to']")));
			Thread.sleep(1200);
			...
			actText = driver
					.findElement(
							By.xpath("//*[@style = 'white-space: nowrap; max-width: 100%; text-overflow: ellipsis;']"))
					.getText();
			if (actText.toLowerCase().contains(expText.toLowerCase())) {
				Object status[] = { "REPAIRS_011_VerifyAssignToQC", actText, expText, "RepairsPage", "Passed",
						java.time.LocalDateTime.now().toString(), env };
				App.values1(status);
			} else {
				Object status[] = { "REPAIRS_011_VerifyAssignToQC", actText, expText, "RepairsPage", "Failed",
						java.time.LocalDateTime.now().toString(), env };
				App.values1(status);
			}
			...
			driver.findElement(By.xpath("//*[@class='side-drawer open']")).findElement(By.xpath("//*[@type='submit']"))
					.click();
			Thread.sleep(1400);
			...
			if (actText.toLowerCase().contains(expText.toLowerCase())) {
				...
				Object status[] = { "REPAIRS_012_VerifyQCCheckList_as_Pass", actText, expText, "RepairsPage", "Passed",
						java.time.LocalDateTime.now().toString(), env };
				App.values1(status);
			} else {
				Object status[] = { "REPAIRS_012_VerifyQCCheckList_as_Pass", actText, expText, "RepairsPage", "Failed",
						java.time.LocalDateTime.now().toString(), env };
				App.values1(status);
			}
			// Repair Report
			...
			if (wCount.size() == 2) {
				...
				driver.close();
				Object status[] = { "REPAIRS_013_Verify_Repair_Report", "Repair Report Working!",
						"Opened Tab count is " + wCount.size(),
						"RepairsPage", "Passed", java.time.LocalDateTime.now().toString(), env };
				App.values1(status);
				driver.switchTo().window(ids[0].toString());
				Thread.sleep(1700);
			} else {
				Object status[] = { "REPAIRS_013_Verify_Repair_Report", "Repair Report Not Working!",
						"Opened Tab count is " + wCount.size(),
						"RepairsPage", "Failed", java.time.LocalDateTime.now().toString(), env };
				App.values1(status);
			}
			...
			App.spinner();
			Thread.sleep(1500);
			String act_comp_text = driver.findElement(By.xpath("//*[@style = 'width: 22%; overflow: unset;']"))
					.getText();
			...
			Object status11[] = { "REPAIRS_010_VerifyQCCheckList_as_Fail", "Not executing in QA environment", "",
					"RepairsPage", "Not Executed..",
					java.time.LocalDateTime.now().toString(), env };
			App.values1(status11);
			Object status12[] = { "REPAIRS_012_VerifyQCCheckList_as_Pass", "Not executing in QA environment", "",
					"RepairsPage", "Not Executed..",
					java.time.LocalDateTime.now().toString(), env };
			App.values1(status12);
			Object status13[] = { "REPAIRS_011_VerifyAssignToQC", "Not executing in QA environment", "", "RepairsPage",
					"Not Executed..",
					java.time.LocalDateTime.now().toString(), env };
			App.values1(status13);
			Object status14[] = { "REPAIRS_013_Verify_Repair_Report", "Not executing in QA environment", "",
					"RepairsPage", "Not Executed..",
					java.time.LocalDateTime.now().toString(), env };
			App.values1(status14);
			Object status[] = { "REPAIRS_014_VerifyCreateSalesOrder_FromRepair", "Not executing in QA environment", "",
					"RepairsPage", "Not Executed..",
					java.time.LocalDateTime.now().toString(), env };
			App.values1(status);
			Object status1[] = { "REPAIRS_015_Verify_Parts_Purchase_Icon_isDisplayed_OrNot",
					"Not executing in QA environment", "", "RepairsPage", "Not Executed..",
					java.time.LocalDateTime.now().toString(), env };
			App.values1(status1);
			Object status2[] = { "REPAIRS_016_VerifyCreateJobFromRepair", "Not executing in QA environment", "",
					"RepairsPage", "Not Executed..",
					java.time.LocalDateTime.now().toString(), env };
			App.values1(status2);
			Object status3[] = { "REPAIRS_031_Verify_Repair_In_Progress", "Not executing in QA environment", "",
					"RepairsPage", "Not Executed..",
					java.time.LocalDateTime.now().toString(), env };
			App.values1(status3);
			Object status4[] = { "REPAIRS_032_VerifyCreate_PartsPurchase", "Not executing in QA environment", "",
					"RepairsPage", "Not Executed..",
					java.time.LocalDateTime.now().toString(), env };
			App.values1(status4);
			Object status5[] = { "REPAIRS_033_Verify_Completed_Button", "Not executing in QA environment", "",
					"RepairsPage", "Not Executed..",
					java.time.LocalDateTime.now().toString(), env };
			App.values1(status5);
		}
	...
	public void quotesModule(String leadTime, String leadValue, String discount, String env) throws Exception {
		...
		App.spinner();
		Thread.sleep(1300);
		String total_count = driver.findElements(By.xpath("//*[contains(@class, 'summary-panel-number')]")).get(2)
				.getText().replace(",", "");
		int all_quotes_count = Integer.parseInt(total_count);
		// Parts Quotes Count
		App.click_xpath("//*[text() = 'Parts Quotes']", "click", "");
		App.spinner();
		Thread.sleep(1300);
		App.clearFilter();
		App.spinner();
		Thread.sleep(1200);
		total_count = driver.findElements(By.xpath("//*[contains(@class, 'summary-panel-number')]")).get(2).getText()
				.replace(",", "");
		int parts_quotes_count = Integer.parseInt(total_count);
		// Repair Quotes Count
		App.click_xpath("//*[text() = 'Repair Quotes']", "click", "");
		App.spinner();
		Thread.sleep(1300);
		App.clearFilter();
		App.spinner();
		...
		if (all_quotes_count == quotes_count) {
			Object status[] = { "QUOTES_033_VerifyAllQuotes_with_Count",
					"All Quotes count in grid is " + all_quotes_count,
					"Parts, Repair and System Quotes count in grid is " + quotes_count,
		...
		quotes.verifyFiltersInQuoteListView("Applied Industrial-Warren", "Dallas House", "Won", "sivakrishna Epi", 1,
				env);
		// Filter's State Maintenance
		// Warning Pop Up
		...
		App.spinner();
		Thread.sleep(1300);
		String org_acc_number = driver.findElements(By.xpath("//*[contains(@class, 'calc-width-33')]")).get(5)
				.findElement(By.tagName("p")).getText();
		String actText = driver.findElement(By.xpath("//*[@class='quote-num-and-status']")).getText();
		String expText = "OPEN";
		if (actText.toLowerCase().contains(expText.toLowerCase())) {
			Object status[] = { "QUOTES_001_VerifyCreateQuote", actText, expText, "QuotesPage", "Passed",
					java.time.LocalDateTime.now().toString(), env };
			App.values1(status);
		} else {
			Object status[] = { "QUOTES_001_VerifyCreateQuote", actText, expText, "QuotesPage", "Failed",
					java.time.LocalDateTime.now().toString(), env };
			App.values1(status);
		}
		List<WebElement> edits = driver.findElement(By.xpath("//*[@id='repair-info-id']"))
				.findElements(By.className("pi-label-edit-icon"));
		// delete quote option
		...
		if (driver.getCurrentUrl().contains("staging")) {
			...
			String bef_quo_type = driver.findElements(By.xpath("//*[contains(@class, 'calc-width-33')]")).get(1)
					.findElement(By.tagName("p")).getText();
			edits.get(0).click();
			Thread.sleep(1200);
			App.click_xpath("//*[text() = 'System Quote']", "click", "");
			Thread.sleep(1300);
			App.click_xpath("//*[text() = 'Parts Quote']", "click", "");
			Thread.sleep(1200);
			App.click_xpath("//*[@title = 'Save Changes']", "click", "");
			Thread.sleep(1200);
			App.spinner();
			Thread.sleep(2000);
			String aft_quo_type = driver.findElements(By.xpath("//*[contains(@class, 'calc-width-33')]")).get(1)
					.findElement(By.tagName("p")).getText();
			if (actText != expText) {
				Object status[] = { "QUOTES_032_VerifyUpdateQuoteType", "before update quote type is " + bef_quo_type,
						"After update quote type is " + aft_quo_type,
						"QuotesPage", "Passed", java.time.LocalDateTime.now().toString(), env };
				App.values1(status);
			} else {
				Object status[] = { "QUOTES_032_VerifyUpdateQuoteType", "before update quote type is " + bef_quo_type,
		...
		expText = driver.findElement(By.id("repair-items")).findElement(By.tagName("h4")).getText();
		if (actText != expText) {
			Object status[] = { "QUOTES_002_VerifySelectItemToQuote", actText, expText, "QuotesPage", "Passed",
					java.time.LocalDateTime.now().toString(), env };
			App.values1(status);
		} else {
			Object status[] = { "QUOTES_002_VerifySelectItemToQuote", actText, expText, "QuotesPage", "Failed",
					java.time.LocalDateTime.now().toString(), env };
			App.values1(status);
		}
		String ven_name_quote_del = driver.findElement(By.xpath("//*[contains(@class, 'm-0 manufacter')]")).getText();
		String stockCode = driver.findElement(By.xpath("//*[@class=' width-25 flexed']")).findElement(By.tagName("h4"))
				.getText();
		// verify GP calculation
		double act_gp_value = Double.parseDouble(driver.findElements(By.xpath("//*[contains(@class, 'w-32')]")).get(0)
				.findElement(By.tagName("h4")).getText().replace("%", ""));
		double act_dis_value = Double
				.parseDouble(driver.findElements(By.xpath("//*[contains(@class,'d-flex align-center g-8 ')]")).get(1)
						.findElement(By.tagName("h4")).getText().replace("%", ""));

		double qp = Double.parseDouble(driver.findElements(By.xpath("//*[contains(@class, 'item-value-ellipsis')]"))
				.get(2).getText().replace("$", "").replace(",", ""));
		double lp = Double.parseDouble(driver.findElements(By.xpath("//*[contains(@class, 'item-value-ellipsis')]"))
				.get(7).getText().replace("$", "").replace(",", ""));
		double iidm_cost = Double
				.parseDouble(driver.findElements(By.xpath("//*[contains(@class, 'item-value-ellipsis')]")).get(4)
		...
		if (act_gp_value == gp_value) {
			Object status[] = { "QUOTES_039_Verify_GP_Value", "actual gp " + act_gp_value, "expected gp " + gp_value,
					"QuotesPage", "Passed",
					java.time.LocalDateTime.now().toString(), env };
			App.values1(status);
		} else {
		...
		if (act_dis_value == dis_value) {
			Object status[] = { "QUOTES_034_Verify_discount_Value", "actual discount " + act_dis_value,
					"expected discount " + dis_value, "QuotesPage",
					"Passed", java.time.LocalDateTime.now().toString(), env };
			App.values1(status);
		} else {
			Object status[] = { "QUOTES_034_Verify_discount_Value", "actual discount " + act_dis_value,
		...
		actText = driver.findElements(By.xpath("//*[@class='d-flex align-center g-16 ']")).get(1)
				.findElement(By.tagName("h4")).getText();
		expText = "TBD";
		if (actText.contains(expText)) {
			Object status[] = { "QUOTES_003_VerifyLeadTimeDisplayedOrNot", actText, expText, "QuotesPage", "Passed",
					java.time.LocalDateTime.now().toString(), env };
			App.values1(status);
		} else {
			Object status[] = { "QUOTES_003_VerifyLeadTimeDisplayedOrNot", actText, expText, "QuotesPage", "Failed",
					java.time.LocalDateTime.now().toString(), env };
			App.values1(status);
		}
		// Edit Icon
		...
		price.clickButton("Save");
		Thread.sleep(2000);
		actText = driver.findElements(By.xpath("//*[contains(@class,'d-flex align-center g-8 ')]")).get(1)
				.findElement(By.tagName("h4")).getText();
		expText = discount + " %";
		if (actText.equals(expText)) {
			Object status[] = { "QUOTES_004_VerifyBulkEdit", "Displayed Discount is " + actText,
					"Applied Discount is " + expText, "QuotesPage",
					"Passed", java.time.LocalDateTime.now().toString(), env };
			App.values1(status);
		} else {
		...
		JavascriptExecutor js = (JavascriptExecutor) driver;
		js.executeScript("arguments[0].scrollIntoView(true);",
				driver.findElement(By.xpath("//*[text() = 'Internal Approvals']")));
		Thread.sleep(2000);
		String total_price_detail = driver.findElements(By.xpath("//*[contains(@class, 'width-auto')]")).get(2)
				.findElement(By.tagName("h4")).getText().replace("$", "").replace(",", "");
		double total_value = Double.parseDouble(total_price_detail);
		System.out.println("total value in int is " + total_value);
		js.executeScript("arguments[0].scrollIntoView(true);",
				driver.findElement(By.xpath("//*[@title='RFQ Received Date']")));
		...
		edits = driver.findElement(By.xpath("//*[@id='repair-info-id']"))
				.findElements(By.className("pi-label-edit-icon"));
		Thread.sleep(2400);
		...
		if (total_value >= 10000) {
			...
			Thread.sleep(1200);
			wait.until(ExpectedConditions
					.invisibilityOfElementLocated(By.xpath("//*[text() ='Submit for Internal Approval']")));
			wait.until(ExpectedConditions
					.visibilityOfAllElementsLocatedBy(By.xpath("//*[@class='quote-num-and-status']")));
			Thread.sleep(2500);
			actText = driver.findElement(By.xpath("//*[@class='quote-num-and-status']")).getText();
			...
			Object status[] = { "QUOTES_005_Verify" + si_apr, "If total price is <=10000 we are't displaying " + si_apr,
					""
							+ "total price is " + total_value,
					"QuotesPage", "Not Executing", java.time.LocalDateTime.now().toString(), env };
			App.values1(status);
		}
		// Approve The Quote
		// Warning Pop Up
		...
		if (revise) {
			...
			Object status[] = { "QUOTES_006_Verify_Display_TheReviseQuote_When_Quote_After_Open_Status",
					"Revise Quote button is Displayed ..? " + revise, "", "QuotesPage", "Passed",
					java.time.LocalDateTime.now().toString(), env };
			App.values1(status);
		} else {

			Object status[] = { "QUOTES_006_Verify_Display_TheReviseQuote_When_Quote_After_Open_Status",
					"Revise Quote button is Displayed ..? " + revise, "", "QuotesPage", "Failed",
					java.time.LocalDateTime.now().toString(), env };
			App.values1(status);
		}
		// Send To Customer
		...
		if (actText.toLowerCase().contains(expText.toLowerCase())) {
			...
			Object status[] = { "QUOTES_009_VerifyQuoteWon", actText, expText, "QuotesPage", "Passed",
					java.time.LocalDateTime.now().toString(), env };
			App.values1(status);
		} else {

			Object status[] = { "QUOTES_009_VerifyQuoteWon", actText, expText, "QuotesPage", "Failed",
					java.time.LocalDateTime.now().toString(), env };
			App.values1(status);
		}
		// Create Sales Order
		...
		if (driver.getCurrentUrl().contains("staging")) {
			...
			if (server) {
				Object status[] = { "QUOTES_010_VerifyCreateSalesOrder_From_Quote", serverMsg, "", "QuotesPage",
						"Failed",
						java.time.LocalDateTime.now().toString(), env };
				App.values1(status);
				price.takesScreenShot("create_sales_order.png");
				...
				wait.until(ExpectedConditions
						.visibilityOfElementLocated(By.xpath("//*[text() = 'Sales Order Information']")));
				String order_num_status = driver.findElement(By.xpath("//*[@class = 'quote-num-and-status']"))
						.getText();
				if (order_num_status.toLowerCase().contains("open order")) {
					Object status[] = { "QUOTES_010_VerifyCreateSalesOrder_From_Quote",
							"order created with " + act_dis_value, "", "QuotesPage", "Passed",
							java.time.LocalDateTime.now().toString(), env };
				} else {
					Object status[] = { "QUOTES_010_VerifyCreateSalesOrder_From_Quote", "order not created", "",
							"QuotesPage", "Failed",
							java.time.LocalDateTime.now().toString(), env };
					App.values1(status);
				}
				...
				if (tabIds.size() == 3) {
					...
					Thread.sleep(1500);
					wait.until(ExpectedConditions
							.invisibilityOfElementLocated(By.xpath("//*[text() = 'Job Information']")));

					if (tabIds.size() == 3) {
						jobId = tabIds.get(2).getText();
						Object status[] = { "QUOTES_011_VerifyCreateJob_From_Quote",
								"Job created with Job Id is " + jobId, "", "QuotesPage", "Passed",
								java.time.LocalDateTime.now().toString(), env };
						App.values1(status);
						tabIds.get(0).click();
						App.spinner();
						...
						Object status[] = { "QUOTES_011_VerifyCreateJob_From_Quote", "Jon Not Created", "",
								"QuotesPage", "Failed",
								java.time.LocalDateTime.now().toString(), env };
						App.values1(status);
					}
					...
					Object status[] = { "QUOTES_011_VerifyCreateJob_From_Quote", "Jon Not Created",
							"count of ids is" + tabIds.size(), "QuotesPage", "Failed",
							java.time.LocalDateTime.now().toString(), env };
					App.values1(status);
				}
			}
			...
			Object status[] = { "QUOTES_010_VerifyCreateSalesOrder_From_Quote", "Not executing in QA environment", "",
					"QuotesPage", "Not Executed..",
					java.time.LocalDateTime.now().toString(), env };
			App.values1(status);
			Object status1[] = { "QUOTES_011_VerifyCreateJob_From_Quote", "Not executing in QA environment", "",
					"QuotesPage", "Not Executed..",
					java.time.LocalDateTime.now().toString(), env };
			App.values1(status1);
		}
	...
	public void approval_questions(double total_value) throws Exception {
		...
		if (total_value >= 50000) {
			...
			App.click_xpath("//*[@placeholder = 'Enter Decision Making Process']", "send_keys",
			...
			App.click_xpath("//*[@placeholder = 'Enter Reasons']", "send_keys",
					"for testing Approval approval questions at");
			...
			act.sendKeys(Keys.ENTER).build().perform();
			App.click_xpath("//*[@placeholder = 'Enter Pain']", "send_keys", "Test Pain");
			App.click_xpath("//*[@placeholder = 'decision_making_process']", "send_keys",
					"Test Decision Making Process");
			driver.findElements(By.xpath("//*[text() = 'Save']")).get(1).click();
			Thread.sleep(2000);
			Thread.sleep(1400);
	...
	public void verify_my_repair_repair_request_tab(String env) throws Exception {
		...
		driver.findElement(By.xpath("//textarea[@placeholder= 'Type here']"))
				.sendKeys("Internal Item Notes While Assign Technician");
		...
		App.spinner();
		Thread.sleep(1300);
		String exp_text = "Pending Evaluation";
		...
		if (result) {
			Object status[] = { "REPAIRS_009_Verify_My_Repair_Requests_Tab",
					"Actual displayed test is " + act_text + " " + rma_number,
		...
		driver.navigate().to(url_repair);
		App.spinner();
		Thread.sleep(1300);
		repair.evaluateItem();
		// Redirect to My repair request tab list view
		driver.navigate().to(url_my_repair);
		App.spinner();
		Thread.sleep(1300);
		try {
		...
		if (result) {
			...
			if (result) {
				Object status[] = {
						"REPAIRS_025_Verify_Quote_Stattus_Pending_Quote_Should not_display_in_My_Repair_Requests_Tab",
						"Actual displayed RMA is " + rma_number,
	...
	public void verify_total_price_grid_detail_view(String env) throws Exception {
		...
		wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//*[text() = 'Add Options']")));
		Thread.sleep(2500);
		String total_price_det = driver.findElements(By.xpath("//*[contains(@class, 'width-auto')]")).get(2)
				.findElement(By.tagName("h4")).getText();
		...
		if (total_price_list_view.equals(total_price_det)) {
			Object status[] = { "QUOTES_027_VerifyPrices_In_Grid_and_Detailed_View_" + quote_number,
					"In grid price " + total_price_list_view,
		...
		App.spinner();
		Thread.sleep(1300);
		App.click_xpath("//*[contains(@src, 'clone')]", "click", "");
		Thread.sleep(1300);
		...
		Thread.sleep(2500);
		String aft_clone_total_price_det = driver.findElements(By.xpath("//*[contains(@class, 'width-auto')]")).get(2)
				.findElement(By.tagName("h4")).getText();
		Thread.sleep(1300);
		App.click_xpath("//*[text() = 'Quotes']", "click", "");
		App.spinner();
		...
		if (aft_clone_total_price_list.equals(aft_clone_total_price_det)) {
			Object status[] = {
					"QUOTES_030_VerifyPrices_In_Grid_and_Detailed_View_After_Clone_" + aft_clone_quote_number,
					"In grid price " + aft_clone_total_price_list,
		...
		for (int i = 0; i < stock_codes.size(); i++) {
			...
			// aft_clone_stock_codes = driver.findElements(By.xpath("//*[@class = ' width-25
			// flexed']"));
		}
		...
		if (res) {
			Object status[] = { "QUOTES_028_Verify_Items_Line_Order_After_Clone",
					"after clone order is " + aft_clone_line_order,
					"before clone order is " + line_order_bef_clone, "QuotesPage", "Passed",
					java.time.LocalDateTime.now().toString(), env };
			App.values1(status);
		} else {
			Object status[] = { "QUOTES_028_Verify_Items_Line_Order_After_Clone",
					"after clone order is " + aft_clone_line_order,
					"before clone order is " + line_order_bef_clone, "QuotesPage", "Failed",
					java.time.LocalDateTime.now().toString(), env };
			App.values1(status);
		}

		if (aft_clone_total_price_det.equals(total_price_det)) {
			Object status[] = { "QUOTES_029_VerifyPrices_In_Grid_and_Detailed_View_After and Before_Clone",
					"after clone total price in det " + aft_clone_total_price_det,
		...
		if (array2.get(0).equals(aft_array.get(0))) {
			if (array2.get(1).equals(aft_array.get(1))) {
				if (array2.get(2).equals(aft_array.get(2))) {
					Object status[] = { "QUOTES_031_Verify_Change_Items_Order",
							"before change the items order " + bef_array,
	...
	public void left_menu_tab_list(String env) throws Exception {
		...
		while (rs.next()) {
			...
			boolean res = false;
			try {
				driver.findElement(By.xpath("//*[text() = '" + tabName + "']")).isDisplayed();
				...
				try {
					...
					wb.until(ExpectedConditions
							.visibilityOfElementLocated(By.xpath("//*[contains(@src, 'vendor_logo')]")));
					App.horizentalScroll();
					driver.findElement(By.xpath("//*[@class = 'ag-react-container']")).isDisplayed();
					Thread.sleep(1300);
					List<WebElement> receiving_list = driver
							.findElements(By.xpath("//*[@class = 'ag-react-container']"));
					for (WebElement element : receiving_list) {
						act_sta = element.getText();
				...
				if (res) {
					Object status[] = { "REPAIRS_0" + tcCount + "_Verify_" + tabName + "_Tab_in_left_menu",
							"actual status " + act_sta,
	...
	public boolean verifyCreateJob(String tcName, String salesOrderId, int count, String env) throws Exception {
		...
		if (count == 1) {
			...
			wait.until(ExpectedConditions
					.visibilityOfElementLocated(By.xpath("//*[contains(@placeholder,'Sales Order ID')]")));
		...
		Object val[] = this.createJob(orderId);
		boolean res = false;
		if ((Boolean) val[1]) {
			...
			if (actJobStatus.toLowerCase().equals(expJobStatus.toLowerCase())) {
				res = true;
				Object status[] = { tcName, actJobStatus, expJobStatus, "JobsPage", "Passed",
						java.time.LocalDateTime.now().toString(), env };
				App.values1(status);
			} else {
				res = false;
				Object status[] = { tcName, actJobStatus, expJobStatus, "JobsPage", "Failed",
						java.time.LocalDateTime.now().toString(), env };
				App.values1(status);
				price.closeIcon();
			}
			...
			res = false;
			Object status[] = { tcName, val[0], "", "JobsPage", "Failed", java.time.LocalDateTime.now().toString(),
					env };
			App.values1(status);
			price.closeIcon();
		}
	...
	public void createPartsPurchase(String tcName, String jobId, int count, String env) throws Exception {
		...
		try {
			...
			wait.until(
					ExpectedConditions.visibilityOfElementLocated(By.xpath("//*[text() = 'Requestor Information']")));
		...
		if (createPp) {
			...
			if (expPpStatus.toLowerCase().contains("requested")) {
				Object status[] = { tcName, "Parts purchase created with Id" + pp_id, expPpStatus, "PartsPurchasePage",
						"Passed", java.time.LocalDateTime.now().toString(), env };
				App.values1(status);
			} else {
				Object status[] = { tcName, "Parts Purchase is not created!", "", "PartsPurchasePage", "Failed",
						java.time.LocalDateTime.now().toString(), env };
				App.values1(status);
				price.closeIcon();
			}
		} else {
			Object status[] = { tcName, serverMsg, "", "PartsPurchasePage", "Failed",
					java.time.LocalDateTime.now().toString(), env };
			App.values1(status);
			price.closeIcon();
		}
